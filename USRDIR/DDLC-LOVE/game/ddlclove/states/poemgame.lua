...
function poemgame()
	...
	-- Freno de seguridad (PS3): entrar al minijuego de poema es una
	-- transición pesada (nuevos assets, nuevo estado completo). Frenar
	-- el autoskip acá para que el jugador entre en modo normal, en vez
	-- de forzar la carga en medio de un skip rápido.
...
function drawPoemGame()
	...
	-- P2b: durante el reparto de carga poemgame() aún no corrió y la lista
	-- de palabras puede estar vacía; estos guards evitan indexar nil.
	if word[menuselected] then
	...
	elseif poemstate == 0 and settings.lang ~= 'eng' then
		...
		if g_system == 'PS3' then
			local yy = 255
			for line in (tr.poemtime..'\n'):gmatch('(.-)\n') do
				if line ~= '' then
					lg.print(line,360,yy)
				end
				yy = yy + 26
			end
		else
