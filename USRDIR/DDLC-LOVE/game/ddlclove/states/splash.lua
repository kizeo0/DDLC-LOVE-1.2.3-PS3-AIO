...
function drawSplash()
	...
	if state == 'splash' then --splash1 (Team Salvato Splash Screen)
		...
		lg.print('DDLC-LOVE '..dversion..' '..dvertype,15,645)
		lg.print('AIO + FIX',15,675)
		lg.print('By KiZeo',15,705)
	...
	elseif state == 'title' then --title (Title Screen)
		...
		lg.draw(gui.sidebar,-720+titlebg_ypos,-65.78)
		drawSplashChar()
		lg.draw(gui.mainbuttons)
		if justmonika_title then
			-- P10: desde el final del Acto 2 (Acto 3+), el titulo muestra
			-- "JUST MONIKA" como en el juego original
			lg.setFont(rifficfont)
			outlineText('JUST MONIKA', 545, 168)
		end
		...
		local cimg = gui.credits_spa
		if settings.lang == 'eng' then cimg = gui.credits_eng end
		lg.draw(cimg,splashx,10)
		menu_draw()
