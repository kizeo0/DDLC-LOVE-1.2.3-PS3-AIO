function loaderPoems()
	if xaload == 0 then
		sfxplay('pageflip')
		-- Nota: el reset del subsistema de audio nativo (snd.Init()) ahora
		-- vive dentro de audioUpdate() en loader/audio.lua -- cubre esta
		-- sección de poemas también, junto con todos los demás cambios de
		-- canción del juego. No hace falta repetirlo acá.
		...
		elseif poem_author == 'sayori' then
			...
			if chapter ~= 5 then
				...
				-- Freno de seguridad (PS3): acá NO hay cambio de audio
				-- (chapter==5), así que el freno normal (via audioUpdate)
				-- no se dispara solo. Frenar el autoskip directo, ya que
				-- sigue siendo una transición pesada (imagen nueva).
