...
function loadNoise()
	-- P3: liberar referencias del ciclo anterior antes de sobrescribir
	-- (si loadNoise corre dos veces sin unloadanimframe en medio, la tanda
	-- vieja de 4 texturas quedaba huérfana)
	unloadanimframe()
	for i = 1, 4 do
...
function loadVignette()
	vignette = lgnewImage('assets/images/cg/vignette.png')
end

function loadYuriGlitch()
	-- P3: igual que loadNoise
	unloadanimframe()
	for i = 1, 4 do
...
function unloadanimframe()
	if next(animframe) then
		for i = 1, #animframe do
			if animframe[i] ~= nil then
				-- P3 FIX: antes solo entraba si era "number" (handles de Vita),
				-- así que en PS3 ni siquiera se soltaban las referencias Lua.
				-- Ahora se suelta la referencia siempre y se libera el handle
				-- nativo solo cuando es un número (plataformas que lo usan).
...
function event_init(etype,arg1,arg2)
	if xaload == 1 then
		if ps3log then ps3log('EVENT '..tostring(etype)) end
		event_last = etype
		...
		-- PRECARGA AUDIO PARA EVENTOS CRÍTICOS
		audioPreloadEvent(etype)
		...
		elseif etype == 'yuri_glitch_head' then
			-- P3 FIX: antes hacía animframe = {} descartando la tabla vieja
			-- entera sin liberar nada; ahora se vacía correctamente.
			unloadanimframe()
...
function event_end(arg1)
	...
	if arg1 == 's_kill' then
		-- P3 FIX: la familia s_kill se ponía en nil sin freeImage, y
		-- s_killzoom ni siquiera se limpiaba. Ahora liberación consistente.
		if type(s_kill) == "number" then Graphics.freeImage(s_kill) end
		if type(s_kill2) == "number" then Graphics.freeImage(s_kill2) end
		if type(s_killzoom) == "number" then Graphics.freeImage(s_killzoom) end
		if type(s_kill_bg) == "number" then Graphics.freeImage(s_kill_bg) end
		if type(s_kill_bg2) == "number" then Graphics.freeImage(s_kill_bg2) end
		if type(s_kill_bgzoom) == "number" then Graphics.freeImage(s_kill_bgzoom) end
		if type(splash_glitch) == "number" then Graphics.freeImage(splash_glitch) end
		if type(exception) == "number" then Graphics.freeImage(exception) end
		s_kill = nil
		s_kill2 = nil
		s_killzoom = nil
		s_kill_bg = nil
	...
	elseif arg1 == 'n_blackeyes' then
		-- P3 FIX: n_eye se cargaba pero jamás se liberaba ni se anulaba
		Graphics.freeImage(n_blackeyes)
		if type(n_eye) == "number" then Graphics.freeImage(n_eye) end
		n_blackeyes = nil
		n_eye = nil
		event_endnext()
	...
	elseif arg1 == 'beforecredits' then
		...
		end_glitch2 = nil
		end_glitch3 = nil
		-- P3 FIX: vignette se cargaba en este evento pero nunca se liberaba
		if type(vignette) == "number" then Graphics.freeImage(vignette) end
		vignette = nil
		-- P3: eventvar2 (event-3.lua) queda referenciando los end_glitch ya liberados
		eventvar2 = nil
		unloadanimframe()
		event_endnext()
	end
	...
	-- ============================================================
	-- P3: barrido final de imágenes de evento retenidas.
	-- Varios eventos terminan vía event_end('next') o sin rama propia,
	-- dejando sus globals cargadas para siempre (sayori_gs, just_monika).
	-- Se limpian según el último evento inicializado (event_last).
	-- NOTA: ml/nl NO se tocan acá -- son los mismos slots que usan los
	-- sprites normales de Monika/Natsuki y limpiarlos podría dejar a un
	-- personaje sin sprite si el script no lo recarga de inmediato.
	-- ============================================================
	if event_last == 'sayori_gs' then
		if type(sayori_gs1) == "number" then Graphics.freeImage(sayori_gs1) end
		if type(sayori_gs2) == "number" then Graphics.freeImage(sayori_gs2) end
		sayori_gs1 = nil
		sayori_gs2 = nil
	elseif event_last == 'just_monika' then
		-- splash se recarga desde cero al volver al título, seguro soltarla
		splash = nil
	elseif event_last == 'n_blackeyes' and arg1 ~= 'n_blackeyes' then
		n_eye = nil
	end
	event_last = nil
end
