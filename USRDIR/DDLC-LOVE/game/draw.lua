...
local lgnewImage_orig = lg.newImage
local imgcache = {}
function lgnewImage(new)
	-- Durante skip en PS3, no recargar imágenes; devolver la cacheada si existe
	if autoskip and autoskip > 0 and g_system == 'PS3' and imgcache[new] then
		return imgcache[new]
	end
	if imgcache[new] then
		return imgcache[new]
	end
	if love.filesystem.getInfo(new) then
		local img = lgnewImage_orig(new)
		imgcache[new] = img
		if ps3log and g_system == 'PS3' then
			ps3log('IMG '..new)
		end
		return img
	end
